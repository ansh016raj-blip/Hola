...
# Use string session (required for this build)
STRING_SESSION = os.getenv("STRING_SESSION")
if not STRING_SESSION:
    log.error("Missing STRING_SESSION environment variable. Set STRING_SESSION to your string session.")
    raise SystemExit(1)
...
# Files (ensure these exist in repo)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_DIR = os.path.join(BASE_DIR, "database")
ST_DIR = os.path.join(BASE_DIR, "storage")
os.makedirs(DB_DIR, exist_ok=True)
os.makedirs(ST_DIR, exist_ok=True)

DB_FILE = os.path.join(DB_DIR, "SlaveDb.json")
MONITORED_FILE = os.path.join(ST_DIR, "nmonitored_groups.json")
...
_ensure_json(DB_FILE, {})
_ensure_json(MONITORED_FILE, {})
...
def clean_name(s: str) -> str:
    # remove bracketed tags, emojis, keep letters/numbers/spaces/dash
    s = re.sub(r"\[.*?\]", "", s)
    s = re.sub(r"[^\w\s-]", "", s)
...
async def spam_loop():
    ...
    while spamming:
        try:
            ...
            log.exception("Spam error, continuing")
...
# ---------------------------
# Auto-grab (global across monitored groups + target)
# ---------------------------
...
async def autograb(event):
    try:
        ...
        if allowed_rarities is not None:
            entry = database.get(pid, {}) if isinstance(database.get(pid), dict) else {}
...
if __name__ == "__main__":
    try:
        ...
        log.info("Stopping...")
        use_random_interval = False
        spamming = False_interval}s" if "spam_interval" in globals() else "n/a")
    lines = [
        "📊 **Status**",
        f"• {status}",
        f"• Mode: `{current_mode}` ({mode_delays[current_mode]}s)",
